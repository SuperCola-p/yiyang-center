...
CREATE TABLE IF NOT EXISTS `nursing_item` (
    ...
    `exec_quantity`   INT            DEFAULT NULL COMMENT '执行次数/数量',
    `exec_times`      INT            DEFAULT NULL COMMENT '周期内执行次数(兼容旧字段)',
...
CREATE TABLE IF NOT EXISTS `nursing_record` (
    ...
    `client_id`          BIGINT   DEFAULT NULL COMMENT '客户ID',
    `nursing_item_id`    BIGINT   DEFAULT NULL COMMENT '护理项目ID',
    `health_assistant_id` VARCHAR(50) DEFAULT NULL COMMENT '护理员登录账号(对应operator.login_code)',
